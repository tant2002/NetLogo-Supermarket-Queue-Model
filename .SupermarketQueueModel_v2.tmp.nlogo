...
; this read read interpleted value out transaction count out of POS data file and return it as arrival rate (lambda function value )  for Non-omogeneous poisson process
  let value 0
  carefully
...
to-report customer-server-service-time-expected ;;customer reporter
...
to-report customer-sco-server-service-time-expected ;;customer reporter
...
to-report server-waiting-time-expected ;;server reporter
  ;report expected waiting time on queue of server
  ifelse is-agent? customer-being-served [
    report (sum ( [ customer-server-service-time-expected ] of  customers with [member? self [server-queue] of myself ])) + (next-completion-time - ticks)
  ][
    report (sum ( [ customer-server-service-time-expected ] of  customers with [member? self [server-queue] of myself ]))
  ]
end

to-report sco-zone-waiting-time-expected
;report expected waiting time on sco-zone.  if no SCO checkout is define it return max run time
  ifelse any? sco-servers with [open] [
    ...
    ifelse next-sco-server != nobody [
      report (sum ( [ customer-sco-server-service-time-expected ] of  customers with [member? self sco-zone-queue ]) / count sco-servers with [open] ) + ( [next-completion-time] of next-sco-server - ticks )
    ][
      report (sum ( [ customer-sco-server-service-time-expected ] of  customers with [member? self sco-zone-queue ]) / count sco-servers with [open] )
    ]
  ][
    report max-run-time]
end

to-report server-zone-waiting-time-expected
;report expected waiting time on server-zone (single queue)
  ...
  if any? servers with [open] [
    ifelse next-server != nobody  [
      set waiting-time-expected  (sum ( [ customer-server-service-time-expected ] of  customers with [member? self server-zone-queue ]) / count servers with [open] ) + ( [next-completion-time] of next-server - ticks )
    ][
      set waiting-time-expected  (sum ( [ customer-server-service-time-expected ] of  customers with [member? self server-zone-queue ]) / count servers with [open] )
    ]
  ]
...
to customer-checkout-queue-pick-strategy4
  let iserver-picked (min-one-of (servers with [open])  [server-waiting-time-expected])

  let server-picked-waiting-time-expected 0
  if iserver-picked != nobody [set server-picked-waiting-time-expected  [server-waiting-time-expected] of iserver-picked ]

  ifelse single-queue? [  ;if single queue for all servers

    if (server-zone-waiting-time-expected <= sco-zone-waiting-time-expected) or (not any? (sco-servers with [open]))[
      set server-zone-queue-picked? TRUE
      ]
    if (server-zone-waiting-time-expected > sco-zone-waiting-time-expected) or (iserver-picked = nobody)[
      set sco-zone-queue-picked? TRUE]
  ][ ;if all servers have sparate queues

    if (server-picked-waiting-time-expected <= sco-zone-waiting-time-expected) or (not any? (sco-servers with [open]))[
      set server-picked iserver-picked]
    if (server-picked-waiting-time-expected > sco-zone-waiting-time-expected) or (iserver-picked = nobody)[
      set sco-zone-queue-picked? TRUE]
  ]
  customer-checkout-queue-join
end
