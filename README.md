...
The number of servers (both service and self-service) is given as parameters. However the avialability of servers depends - like in real supermarket -  on number of  cashiers present in store. The latter is dependent on workschedule (historical data) and planned work time given as parameter. Because in real environments, cashiers can perform other than checkout tasks in periods of low traffic, the a mechanism for leaving the checkouts and entering the backoffice zone has been implemented. Cashier is triger: to close or to open checkout when the mean queue length fall below or exceed given thresholds respectively. It is also assumed that changeover from backoffice to checkout of cashier takes time (set as parameter). 
### queue discipline
